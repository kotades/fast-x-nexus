...
import { coordinateToH3Zone, getZoneNeighbors, calculateDistanceKm, hasCrossedTelemetryThreshold } from '../lib/h3';

console.log('🧪 Starting H3 Spatial Engine Unit Tests...');

try {
  // Test 1: coordinateToH3Zone maps correctly
  // Coordinate in Lagos, Nigeria (Lekki Phase 1)
  const lat = 6.4281;
  const lng = 3.4219;
  const result = coordinateToH3Zone(lat, lng, 7);
  
  assert.strictEqual(result.isValid, true, 'H3 cell should be valid');
  ...
  // Test 2: getZoneNeighbors matches adjacent hexagons
  ...
  // Test 3: calculateDistanceKm great-circle distance
  // Lekki Phase 1 to Victoria Island (approx 2.4 km)
  ...
  console.log('✅ Test 3 Passed: calculateDistanceKm calculates correct great-circle distance.');

  // Test 4: hasCrossedTelemetryThreshold 50m filter logic
  const p1 = { lat: 6.4281, lng: 3.4219 };
  // ~10 meters away (approx 0.00009 degrees lat)
  const p2 = { lat: 6.4281 + 0.00009, lng: 3.4219 };
  // ~60 meters away (approx 0.00055 degrees lat)
  ...
  console.log('🎉 All H3 Spatial Engine Unit Tests Passed successfully!');
} catch (error) {
